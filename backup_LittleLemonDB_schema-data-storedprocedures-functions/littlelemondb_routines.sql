...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddBooking`(IN booking_id INT, IN customer_id INT, IN booking_date DATETIME, IN table_number INT)
BEGIN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddValidBooking`(IN booking_date DATETIME, IN table_number INT)
BEGIN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `CancelOrder`(IN id_to_delete INT)
BEGIN
	DECLARE order_count INT DEFAULT 0;
	SELECT COUNT(*) INTO order_count FROM orders WHERE OrderID = id_to_delete;
...
BEGIN
	DECLARE table_count INT DEFAULT 0;
	SELECT COUNT(*) INTO table_count FROM booking
	WHERE BookingDate = booking_date AND TableNumber = table_number;
	IF table_count > 0 THEN
